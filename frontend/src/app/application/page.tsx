...
export default function ApplicationPage() {
  ...
  const [formData, setFormData] = useState<FormData>({
    name: "", applicantId: "", gender: "", dateOfBirth: "", age: 21, employeeOrStudent: "", educationLevel: "",
    fieldOfStudy: "", cgpaOrPercentage: "", familyIncome: "",
    familyGovtEmployment: "", sectorPreferences: [],
  ...
  const validateStep = (step: number): boolean => {
    ...
    switch (step) {
      case 1:
        // Minimal checks for step 1
        if (!formData.name) stepErrors.name = "Name is required";
        break;
      case 2:
        // Minimal checks for step 2
  ...
  const handleSubmit = async () => {
    ...
    // Map frontend fields to backend engine payload
    ...
    try {
      const normalizedLocation = (() => {
        const raw = formData.locationPreference || "";
        if (raw.includes("/")) {
      ...
      const payload = {
        candidate: {
          Applicant_ID: formData.applicantId && formData.applicantId.trim() ? formData.applicantId.trim() : undefined,
          Name: formData.name || undefined,
          Skills: (formData.skills || []).join(", "),
          Sector_Preferences: (formData.sectorPreferences || []).join(", "),
          Location_Preference: normalizedLocation,
          Background: formData.employeeOrStudent ? (formData.employeeOrStudent === "Yes" ? "Urban" : "Rural") : "",
          cgpa_or_percentage: formData.cgpaOrPercentage ? Number(String(formData.cgpaOrPercentage).replace('%','')) : 0,
          Education_Level: mapEducation(formData.educationLevel || ""),
        },
        top_n: 5,
      };

      // Save application data as well (useful for debug/fallback)
      ...
      // Persist recommendations for the recommendations page
      ...
      setIsLoading(false);
      // Fallback: proceed to recommendations page where mock/fallback will appear
      router.push('/recommendations');
    }
  };
          ...
          <AnimatePresence mode="wait">
            {currentStep === 1 && (
              ...
              >
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Applicant ID (Optional)</label>
                  <input
                    type="text"
                    value={formData.applicantId}
                    onChange={(e) => handleInputChange("applicantId", e.target.value)}
                    className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors border-gray-300"
                    placeholder="e.g., APP004"
                  />
