...
export default function ComparisonPage() {
  ...
  useEffect(() => {
    const matchIds = (searchParams.get('matches')?.split(',') || []).map(s => String(s));
    setSelectedMatches(matchIds);

    // Try to use recommendations from localStorage for real comparison
    try {
      ...
      if (results.length > 0) {
        ...
        for (const r of results) {
          const id = String(r?.Internship_ID ?? '');
          if (id) mapById[id] = r;
        }
        const mapped: ComparisonMatch[] = matchIds
          .map((id) => mapById[id])
          .filter(Boolean)
          .map((r: any, idx: number) => {
            ...
            // Use backend skills_matched if present
            let skillsMatched: string[] = Array.isArray(r?.skills_matched) ? r.skills_matched : [];
            if (skillsMatched.length === 0 && typeof r?.Skills_Required === 'string') {
              try {
                skillsMatched = r.Skills_Required
                  .replace(/^\[/, '')
                  .replace(/\]$/, '')
                  .split(',')
            ...
            const requirements = skillsMatched.slice(0, 5);
            return {
              ...
              skillsMatched,
              companyType,
              pros: ['Relevant sector exposure', 'Mentorship'],
              cons: ['Limited positions'],
              requirements,
              workCulture: 'Professional, structured processes',
              careerGrowth: 'Good exposure and networking',
              learningOpportunities: ['On-the-job learning', 'Industry practices']
            } as ComparisonMatch;
          });
    ...
    // Fallback to mock data if nothing found in storage
  ...
  const addMatch = (matchId: string) => {
    if (selectedMatches.length < 3 && !selectedMatches.includes(matchId)) {
      ...
      // Try to fetch from stored recs map; if missing, fallback to mock
      try {
        const raw = typeof window !== 'undefined' ? localStorage.getItem('recommendations') : null;
        const parsed = raw ? JSON.parse(raw) : null;
        const results: any[] = Array.isArray(parsed?.results) ? parsed.results : [];
        ...
        if (r) {
          ...
          const companyType: "ministry" | "psu" | "department" = industryType === 'psu' ? 'psu' : 'department';
          let skillsMatched: string[] = Array.isArray(r?.skills_matched) ? r.skills_matched : [];
          const item: ComparisonMatch = {
            id: String(r?.Internship_ID),
            companyName: String(companyFromTitle),
            companyLogo: 'https://www.logopeople.in/wp-content/uploads/2013/01/government-of-india.jpg',
            roleName: title,
            location: String(r?.Location || 'India'),
            ...
            pros: ['Relevant sector exposure', 'Mentorship'],
            cons: ['Limited positions'],
            requirements: skillsMatched.slice(0, 5),
            workCulture: 'Professional, structured processes',
            careerGrowth: 'Good exposure and networking',
            learningOpportunities: ['On-the-job learning', 'Industry practices']
          };
          setComparisonData([...comparisonData, item]);
        } else if (mockMatches[matchId]) {
          setComparisonData([...comparisonData, mockMatches[matchId]]);
        }
      } catch {
        if (mockMatches[matchId]) setComparisonData([...comparisonData, mockMatches[matchId]]);
      }
    }
  };
